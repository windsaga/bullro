...
└──────────┬───────────┘  └───────────┬──────────┘
           ...
           └──────────┬───────────────┘
                      ▼  top 1~3 토픽
           ┌──────────────────────────┐
           │ 게시 이력 유사도 검사    │  ← embedding cosine < 0.75만 통과
           └──────────┬───────────────┘
                      ▼
       ┌──────────────┴───────────────┐
       ▼ (토픽당 반복)                │
...
│   ├── posts.json                   ← 게시 이력 DB (url, title, embedding, score)
...
| **독창성** | 10 | 기존 게시글 embedding cosine < 0.75 |
...
| Embedding (중복 탐지) | sentence-transformers (로컬) | **무료** | NAS에서 로컬 실행 |
...
- [ ] 루브릭 자동 채점 + 분기 게이트 로직
- [ ] sentence-transformers 로컬 embedding 중복 탐지 (NAS JSON DB)
...
def run_pipeline():
    ...
    scored = score_and_deduplicate(
        ...
        history_path=cfg.POSTS_JSON,          # 기존 게시글 임베딩과 비교
...
- **2026-04-28 v5** — 핵심 설계 변경
  ...
  - Embedding: Gemini API → sentence-transformers 로컬 실행
